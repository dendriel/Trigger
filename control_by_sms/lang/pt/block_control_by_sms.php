<?php
	$string['pluginname'] = "Control by SMS";
	
	#$string['daemon_configuration'] = "<html><body><b>-Daemon Configuration-</b></body></html>";
	$string['allow_to_use'] = 'Permitir aos professores utilizarem o serviço de SMS?';

	$string['headerconfig'] = "Sei la ";
	$string['descconfig'] = "Sei la2 ";

	$string['label_open_to_use'] = "Permitir aos professores utilizarem o serviço de SMS?";
	$string['desc_open_to_use'] =  "Use esta opção para permitir ou negar o uso do serviço de SMS elo fórum.";

// Global menu Strings //
 $string['schedule_sms_menu'] = "Agendar SMS";
 $string['reports_menu'] = "Relatórios";
 $string['footnote'] = "Desenvolvido por Vitor Rozsaaaaa";
 $string['service_stopped'] = "Serviço interrompido. Entre em contato com o administrador para mais informções.";

// Status Strings //
 $string['conection_failed'] = "Conexão com o banco de dados falhou!";
 $string['not_allowed'] = "Você não tem permissão para utilizar esta aplicação!";
 $string['daemon_error'] = "Falha no processamento da requisição! Entre em contato com o administrador do sistema."; // daemon error // 
 $string['requisition_ok'] = "A requisição foi registrada com sucesso!";

// Reports Strings //
 $string['active'] = "Ativos";
 $string['canceled'] = "Cancelados";
 $string['failed'] = "Falhos";
 $string['sent'] = "Enviados";
 $string['reports_title'] = "Relatórios";

// Reports Colors //
 $string['green_header'] = "6c9";
 $string['green_body'] = "6f9";
 $string['yellow_header'] = "fc9";
 $string['yellow_body'] = "ff9";
 $string['red_header'] = "f66";
 $string['red_body'] = "f99";
 $string['blue_header'] = "6cf";
 $string['blue_body'] = "6ff";

// Reports cols //
 $string['origin_col'] = "Origem";
 $string['message_col'] = "Mensagem";
 $string['blow_col'] = "Envio";
 $string['destination_col'] = "Destinos";

